...
> [!WARNING]
> **Bu betiği kullanmadan önce aşağıdakileri okuyun.**

Bu betik, Keenetic router üzerinde **sistem düzeyinde yapılandırmalar** gerçekleştirir:

- `aria2c` arka plan servisi başlatır ve durdurur
- `/opt/etc/` altında config, hook ve init dosyaları oluşturur/siler
- **RPC API'yi ağ üzerinde açar** — varsayılan port `6800`, tüm arayüzlerden erişilebilir
- **AriaNg WebUI'yi ağ üzerinde yayınlar** — varsayılan port `6880`, yerel ağdan erişilebilir
- `iptables` kurallarını ve lighttpd web sunucusunu yönetir
- USB disk üzerindeki dosyaları okur ve yazar

### 🔐 Güvenlik Uyarıları

| Risk | Açıklama | Önlem |
|---|---|---|
| **RPC portu açık** | Port 6800 LAN'dan erişilebilir | Güçlü bir RPC Secret Key belirleyin |
| **WebUI portu açık** | Port 6880 LAN'dan erişilebilir | Güvenilmeyen ağlarda kullanmayın |
| **Secret Key** | Zayıf veya boş key güvenlik açığı oluşturur | Script otomatik 24 haneli key üretir, değiştirmeyin |
| **Telegram Token** | Bot token ve Chat ID hassas bilgidir | Kimseyle paylaşmayın, config dosyasını koruyun |
| **İndirilen dosyalar** | Script URL'den gelen içeriği doğrulamaz | Güvenilir kaynaklardan indirme yapın |

### ⚖️ Kullanım Koşulları

- Bu betik **olduğu gibi (as-is)** sunulmaktadır, **hiçbir garanti verilmemektedir**
- Yanlış yapılandırma bağlantı kesintilerine veya sistem sorunlarına yol açabilir
- **Kullanım tamamen kullanıcının sorumluluğundadır**
- Geliştirici, bu betiğin kullanımından doğabilecek doğrudan veya dolaylı zararlardan sorumlu tutulamaz
- Router'ınızda yapılan değişikliklerden önce **yedek almanız** önerilir (B) Yedek & Geri Yükleme)
...
Bu projenin geliştirilmesinde [RevolutionTR](https://github.com/RevolutionTR)'ın **[keenetic-zapret-manager](https://github.com/RevolutionTR/keenetic-zapret-manager)** projesinden ilham alınmıştır.

Genel script mimarisi, özellik seti ve menü yapısı tasarlanırken söz konusu proje referans olarak incelenmiştir. Emeğine ve açık kaynak katkısına teşekkürler. 🤝
